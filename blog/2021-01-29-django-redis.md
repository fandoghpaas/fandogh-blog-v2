...
یکی از مواردی که در مورد وب‌سایت‌های داینامیک یا به زبان فارسی، پویا باید در نظر گرفته شود خود موضوع پویایی‌ است. در دنیای جدید، پشت وب‌سایت‌ها و اپلیکیشن‌ها میزان قابل توجهی عملیات سروری نهفته است. از پرس‌و‌جوهای )Query( سمت دیتابیس گرفته تا Render شدن تمپلیت‌ها و محاسبات ریاضی و منطقی پیچیده.<br/><br/>
شاید مقیاس‌های کوچک و متوسط که ترافیک ورودی خیلی زیادی ندارد، احساس نیاز به این ویژگی را درک نکرده باشند؛ اما رفته رفته در مقیاس‌های بالاتر از متوسط، زمانی که ترافیک‌ ورودی رو به افزایش رفته و درخواست‌های تکراری، با پردازش‌های هزینه‌بر و سنگین سمت سرور افزایش یابند این نیاز احساس شود.
...
در تصویر بالا با فرض اینکه سازوکاری برای **`Cache`** کردن وجود نداشته باشد، همانطور که مشاهده می‌کنید، هر درخواست )درخواست‌ها یکسان هستند( از سمت کاربر مستقیما به سمت دیتابیس منتقل شده و عملیات هزینه‌بر دیتابیسی انجام شده و پاسخ به کاربران بازگردانده می‌شود.
...
همانطور که در تصویر بالا مشاهده می‌کنید، اولین درخواست که از کاربر **Client 1** برای سرور فرستاده می‌شود، سرور ابتدا `Cache` را بررسی می‌کند، به دلیل اینکه نتیجه در `Cache` وجود ندارد، این درخواست به سمت دیتابیس ارسال شده و پاسخ بازگردانده شده ابتدا در `Cache` ذخیره شده، سپس به کاربر ارجاع داده می‌شود.
...
### کاهش هزینه دیتابیس‌
هر Instance از Cache قابلیت اجرای چند صد هزار IOPS یا همان )input/output operations per second( را دارد و که توان برابری با قابلیت اجرای IOPS چندین Instance از دیتابیس را دارد!
...
ممکن است در طول چرخه حیات یک سیستم، شرایط خاصی بوجود بیاید که پیشبینی کارکرد و بهینگی کارایی سیستم امری سخت شود. برای مثال شرایطی مانند روز انتخابات یا روز انتخاب واحد یا جمعه سیاه و … که در طول سال ممکن است به تعداد انگشتان یک دست هم رخ ندهند!<br/>
...
برای شفاف‌تر شدن این تیتر، پروفایل یک celebrity خاص را در نظر بگیرید، به طور متوسط یک پروفایل بخشی از داده‌ها که پتانسیل تغییر بالایی داشته باشد نیست اما مراجعه به آن به تکرار صورت می‌گیرد.<br/>
...
یکی از محبوب‌ترین سرویس‌ها در حوزه `Caching` سرویس **`Redis`** است که در این بلاگ پست برای آموزش در نظر گرفته‌ایم.

### Redis چیست؟
با توجه به تعریف سایت [Redis][redis_site] این سرویس به شرح زیر معرفی شده است:

:::important تعریف Redis
سرویس **Redis** یا **Remote Dictionary Server** یک ساختمان داده مبتنی بر حافظه داخلی و دیتابیس مبتنی بر key-value در حافظه داخلی توزیع شده و همچنین یک سیستم Cache و Message Broker است با قابلیت تاب آوری انتخابی است.
سرویس **Redis** گستره زیادی از تعریف داده‌ها مانند رشته‌ها )Strings(، لیست، Maps، Sets، HyperLogLogs و … را پوشش داده و از آن‌ها پشتیبانی می‌کند.
:::

حال که با تعریف سرویس `Redis` آشنا شدید، وقت آن رسیده تا این سرویس محبوب را راه‌اندازی کنیم.

## ایجاد پروژه تست

برای ایجاد یک پروژه تست با قابلیت Cache از سرویس‌های زیر استفاده می‌کنیم:
-   [پروژه جانگو )Django Project(][django_source_deployment]
-   [MySQL][mysql_managed_services_url]
-   [Redis][redis_managed_services_url]
...
مانند قسمت قبل، برای ساخت سرویس دیتابیس `MySQL` نیز می‌توانیم از [سرویس مدیریت شده][mysql_managed_services_url] موجود بر روی سکو استفاده کنیم.
...
برای آنکه پروژه جانگو بتواند به سرویس Redis و MySQL متصل شود، باید تنظیمات خاصی را در پروژه ایجاد کنیم.

ابتدا باید پکیج مربوط به [django-redis][django_redis_url] و همینطور رابط [mysqlclient][mysql_client] را در فایل requirements.txt قرار دهیم.
...
شما می توانید نسخه پایتونی که استفاده می‌کنید را مشخص کنید؛ سکوی ابری فندق به صورت پیشفرض این نسخه را ۳.۷ در نظر می‌گیرد که برای پروژه ما مورد استفاده است، پس نیازی به وارد کردن نسخه دیگر نیست و با فشردن دکمه Enter به مرحله بعد می رویم.
...
سکوی ابری فندق به صورت خودکار WSGI‌های موجود در پروژه جانگویی شما را تشخیص و به شما پیشنهاد می‌دهد؛ در صورتی که ماژول به درستی نمایش داده شده است، کافی است همان اسم را وارد نمایید در غیر این صورت باید نام ماژول WSGI مورد نظر را وارد نمایید.
...
در این مرحله fandogh-cli یک مانیفست با نام `fandogh.yml` در محل پروژه شما ایجاد کرده است که می‌توانید محتوای آن را مشاهده کنید.

قبل از آنکه سرویس را ایجاد کنیم، نیاز داریم تا چند Environment Variable مربوط به MySQL را در آن وارد کنیم.

به همین منظور مانیفست `fandogh.yml` را با ادیتوری دلخواه باز کرده و مقادیر زیر را در آن قرار دهید نا مانیفست شما شبیه مانیفست زیر شود:
...
این دستور به صورت خودکار پروژه را فشرده کرده، موارد غیر ضروری که نباید آپلود شوند را حذف و بعد از آپلود پروژه، آن را داکرایز کرده و از روی پروژه داکرایز شده به صورت خودکار یک ایمیج با تگ latest ایجاد می‌کند.
...
بعد از آنکه سرویس به درستی مستقر شد، می‌توانید از طریق آدرسی که در اختیار شما قرار داده شده است، سرویس خود را در مرورگر مشاهده بفرمایید. ساختار آدرس سرویس به شکل زیر خواهد بود:
...
اگر این کلید وجود داشته باشد، آنگاه داده‌های متناظر را مستقیما از روی `Cache` را با استفاده از دستور زیر فراخوانی می‌کند؛ این عمل باعث می‌شود درخواست سمت دیتابیس نرفته و در نتیجه بار از روی دیتابیس `MySQL` برداشته شود.
